...
module.exports = {
    log: async (output, embedData = new MessageEmbed) => {
        ...
        if (output.length) console.log(chalk.whiteBright(`[${timeFormatted} - INFO]`, output));
    ...
    warn: async (output, embedData = new MessageEmbed) => {
        const timeFormatted = new Date().toLocaleTimeString("ru-RU", { hour12: false });
        if (output.length) console.log(chalk.yellowBright(`[${timeFormatted} - WARN]`, output));
    ...
    error: async (output, embedData = new MessageEmbed) => {
        const timeFormatted = new Date().toLocaleTimeString("ru-RU", { hour12: false });
        if (output.length) console.log(chalk.redBright(`[${timeFormatted} - ERROR]`, output));
